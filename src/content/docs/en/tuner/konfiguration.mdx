...
The [wiring diagrams](../verkabelung/) show the connections. The [reception overview](../) covers multiple satellites, single-cable systems, motors and cable/aerial reception. Use the data for your own installation.

## Example: one satellite cable for Astra 19.2° East

This example uses a conventional satellite connection with one cable supplying one orbital position. It does not cover Unicable, motorised dishes or FBC configuration.

<Capture id="reception-menu" language="en" alt="Reception menu with tuner configuration, tuner settings and automatic and manual scans" caption="Tuner Configuration opens connection settings. Tuner Settings is a separate group of general options." />

Select the tuner that is physically connected. The tested connection uses **Configuration mode: Simple**, **Mode: Single** and **Satellite: 19.2E Astra**. The additional satellite names may change when the satellite list is updated.

<Capture id="satellite-settings" language="en" alt="Satellite tuner settings with simple configuration, single mode and Astra 19.2 degrees East" caption="A simple configuration for one satellite cable. The tuner letter only identifies the selected input in this test installation." />

**Send DiSEqC** controls switching signals sent to the reception installation. Set it according to your connection. The image shows existing test values, not a preset for every installation. Do not copy a connected tuner's configuration onto additional unconnected inputs.
...
Connections: [wiring diagrams](../verkabelung/) and [reception overview](../). Details and buttons: [automatic scan](../automatischer-suchlauf/), [manual scan](../manueller-suchlauf/) and [signal finder](../signalfinder/).
