...
<Capture id="network-menu" language="en" alt="Network menu with Network Overview selected" caption="This menu leads to adapters, network shares and DNS settings." />
...
<Capture id="lan-settings" language="en" alt="Enabled Ethernet adapter with DHCP Yes, DNS override No and link speed Auto" caption="A working configuration using automatic addressing. The adapter name may differ." />

With DHCP enabled, this dialog hides the manual IPv4 address fields. The network overview shows the assigned addresses. **IP protocol** selects IPv4, IPv6 or both; **DNS override** enables custom DNS settings for this adapter.
...
Continue: [network overview](../), [manual IP](../manuelle-ip/), [DNS](../dns/), [adapter/network restart](../neustart-test/) and [OpenWebif](../openwebif/).
