...
## Understand feed categories

<Capture id="plugin-feed" language="en" alt="Loaded plugin feed with categories including extensions, picons, settings and skins" caption="The loaded package catalogue. Available packages and totals vary with the image and feed." />

The plus sign expands a category. **Extension Packages** provides additional functions, **Skin Packages** changes the interface, **Picon Packages** supplies channel logos and **Setting Packages** contains prepared channel lists. Expanding a category does not install a package.

Before confirming, read the package name and the pending actions. A visible feed confirms that the package list loaded; an installation is confirmed by its own completion message.
