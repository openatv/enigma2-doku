...
## Empfang und Verkabelungszeichnungen

Die zusätzlichen Empfangsreferenzen verwenden den oben genannten OpenATV-Quellstand sowie AutoBouquetsMaker `ba6010b90b57e7ae9f72efbf283955c880c9351c` und Blindscan `35aed6fadf8d2bd2c125237db675d8bae49f349a`. Ursprüngliche Autoren und Lizenzen bleiben den jeweiligen Projekten zugeordnet. Quellen, Felder und SHA-256 sind in `data/reception-settings.json` erfasst; Original-Hilfetexte werden durch Erläuterungen ergänzt.

Die 16 SVG-Verkabelungsbilder wurden für dieses Handbuch neu erstellt und gehören zum Dokumentationsquelltext. `scripts/create-reception-diagrams.py` erzeugt sie reproduzierbar. Das Foto `src/assets/reception/spaun-tar5.png` wurde vom Betreiber zur Verwendung bereitgestellt und unverändert übernommen. SPAUN-Marke und Produktkennzeichen verbleiben bei ihren Rechteinhabern. `data/reception-figures.json` dokumentiert Herkunft und Prüfsumme. Der verlinkte Herstellerkatalog wird nicht mit der Website ausgeliefert; die Zeichnungen sind eigenständige Erklärungen der Signalwege.

38 weitere native Enigma2-Bilder wurden mit ScreenshotTour `92de9f0dd414bbf2bf0eb021ea230955b5cfe950` erfasst, einzeln geprüft und mit ihren Manifest-Prüfsummen importiert. Der native Gesamtbestand beträgt damit 370 Bilder. Beispielwerte sind als solche gekennzeichnet; kein Empfangstest fremder Anlagentopologien wird daraus abgeleitet.

## Ergänzung: Netzwerk, OpenWebif und Zugang

21 Kapitel je Sprache wurden redaktionell anhand der OpenATV-Netzwerk-/CI-/Softcam-/Jugendschutzquellen sowie OpenWebif 2.4.0 (`cacd3f69d065fb0eae326aebf456273dc957d2fa`) erstellt. Quellen/Hashes stehen in `data/network-security-sources.json`; `data/network-services.json` dokumentiert 22 Dienste und `data/openwebif-settings.json` die 23 Hauptsetupfelder. Upstream-Code wird nicht mit der Website ausgeliefert. Die Beschreibung eines externen Softcam-Feeds ist keine Lieferung oder Unterstützung seiner Pakete durch OpenATV.

26 weitere native DE/EN-Bilder stammen aus ScreenshotTour `b9b260f88baad3d12ee676e6383c53c4c4c009ab`, einzeln geprüft und mit Manifest-Hashes importiert. Zusätzlich zeigen 16 Browserbilder unter `src/assets/openwebif/` die unveränderten Classic-/Modern-Oberflächen und vorhandenen Editoren. Herkunft und Prüfung: `data/openwebif-captures-review.json`. Die Oberfläche von OpenWebif unterliegt ihrer Upstream-Lizenz GPL-3.0; Senderlogos, Sendungstitel, EPG-Texte und Kennzeichen verbleiben bei den jeweiligen Rechteinhabern. Die Browserbilder dienen der Erläuterung der Bedienung; keine Film-/Senderwiedergabe wurde dafür gestartet.

## Ergänzung: Bedienung / Oberfläche

Die 28 zusätzlichen nativen Enigma2-Aufnahmen (DE/EN, `ui-*`) stammen aus der autorisierten OpenATV-Testumgebung und wurden mit gestoppter Wiedergabe/Bootlogo aufgenommen. Formularbeispiele sind als ungespeichert gekennzeichnet. Bestehende Infobar-, Skin-, Menü- und Kanallistenbilder werden weiterverwendet. Die ergänzenden Beschreibungen beruhen auf den in `data/usage-gui-coverage.json` dokumentierten OpenATV-Quellen; deren bestehende Lizenzen bleiben maßgeblich.
